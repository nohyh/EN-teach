...
export default function FamilyApp({ apiBaseUrl }: { apiBaseUrl: string; dom?: import("expo/dom").DOMProps }) {
  ...
  return <main className="family-app">
    ...
    <nav className="family-mobile-nav" aria-label="家长端手机版导航"><button onClick={() => document.querySelector(".family-metrics")?.scrollIntoView({ behavior: "smooth" })}><span>周</span>周报</button><button onClick={() => document.querySelector(".learning-policy-card")?.scrollIntoView({ behavior: "smooth" })}><span>控</span>规则</button><button onClick={() => document.querySelector(".notifications-card")?.scrollIntoView({ behavior: "smooth" })}><span>信</span>通知</button></nav>
    <div className="family-layout">
      ...
      <section className="family-report">
        ...
        <section className="family-card notifications-card"><h2>站内通知</h2>{notifications.map((item) => <button className={`notification-row ${item.read_at ? "read" : "unread"}`} key={item.id} onClick={() => void markRead(item.id)}><span><strong>{item.title}</strong><small>{item.body}</small></span><em>{item.read_at ? "已读" : "标为已读"}</em></button>)}{!notifications.length && <p>目前没有新通知。</p>}</section>
        <section className="family-card preferences-card"><h2>通知偏好</h2><p>默认关闭连续签到提醒，减少不必要的学习焦虑。</p>{preferences && (Object.keys(preferenceLabels) as Array<keyof Preferences>).map((key) => <label className="preference-row" key={key}><span>{preferenceLabels[key]}</span><input type="checkbox" checked={preferences[key]} onChange={() => void updatePreference(key)} /></label>)}</section>
      </section>
    </div>
  </main>;
}
