...
type OutboxSnapshot = { summary: { pending: number; failed: number; sent: number; dead: number }; items: Array<{ id: number; channel: string; status: string; attempt_count: number; max_attempts: number; last_error?: string }> };
...
export default function ContentAdmin({ apiBaseUrl }: { apiBaseUrl: string; dom?: import("expo/dom").DOMProps }) {
  ...
  const [mobileSection, setMobileSection] = useState<"content" | "assignments" | "classes" | "reports">("content");
  ...
  const refresh = useCallback(async () => {
    ...
    if (operatorRole === "admin") setOutbox(await call("/api/v1/admin/notifications/outbox") as OutboxSnapshot);
    else setOutbox(null);
  }, [call, operatorRole, token]);
  ...
  const dispatchNotifications = async () => {
    try {
      ...
      const delivery = await call("/api/v1/admin/notifications/outbox/process", { method: "POST" }) as { sent: number; failed: number; dead: number };
      setNotice(`通知派发 ${result.created} 条，已发送 ${delivery.sent} 条，失败 ${delivery.failed + delivery.dead} 条`);
      setOutbox(await call("/api/v1/admin/notifications/outbox") as OutboxSnapshot);
    } catch (error) { setNotice(displayError(error)); }
  };
  ...
  return <main className={`content-admin mobile-section-${mobileSection}`}>
    <header className="admin-top"><div><span>CONTENT STUDIO</span><h1>教材与教学运营工作台</h1><p>{operator}</p></div><nav>{operatorRole === "admin" && <button onClick={() => void dispatchNotifications()}>派发并发送</button>}<a href="/">学生端</a><button onClick={() => { setToken(""); setOperator(""); setOperatorRole(""); }}>退出</button></nav></header>
    {notice && <div className="notice" role="status">{notice}<button onClick={() => setNotice("")}>×</button></div>}
    <nav className="mobile-ops-nav" aria-label="教师端手机版导航"><button className={mobileSection === "content" ? "active" : ""} onClick={() => setMobileSection("content")}><span>▤</span>内容</button><button className={mobileSection === "assignments" ? "active" : ""} onClick={() => setMobileSection("assignments")}><span>✓</span>作业</button><button className={mobileSection === "classes" ? "active" : ""} onClick={() => setMobileSection("classes")}><span>班</span>班级</button><button className={mobileSection === "reports" ? "active" : ""} onClick={() => setMobileSection("reports")}><span>↗</span>数据</button></nav>
    <section className="admin-grid">
      <aside className="content-ops">
      ...
      <section className="workspace">
        {outbox && <section className="admin-card outbox-card"><div className="card-title"><span>送</span><div><h2>通知发送队列</h2><p>开发环境通过模拟通道验证；生产可切换邮件与 APNs</p></div><button className="quiet" onClick={() => void dispatchNotifications()}>处理队列</button></div><div className="outbox-metrics"><div><strong>{outbox.summary.pending}</strong><small>待发送</small></div><div><strong>{outbox.summary.failed}</strong><small>待重试</small></div><div><strong>{outbox.summary.sent}</strong><small>已发送</small></div><div><strong>{outbox.summary.dead}</strong><small>人工处理</small></div></div>{outbox.items.filter((item) => item.status === "failed" || item.status === "dead").slice(0, 5).map((item) => <p className="outbox-error" key={item.id}>{item.channel} · {item.attempt_count}/{item.max_attempts}：{item.last_error}</p>)}</section>}
