...
# PCM/WAV 按 16kHz * 16bit * 1 声道的 20ms 帧实时发送。
PCM_FRAME_BYTES = 16000 * 2 * 20 // 1000
COMPRESSED_AUDIO_CHUNK_BYTES = 4 * 1024
AUDIO_CHUNK_INTERVAL_S = 0.02
...
class SsecpClient:
    ...
    def _packet_sig(self, timestamp: int) -> str:
        """WebSocket connect/start 包的 sig 字段, sha1(appid + secret + ts)"""
    ...
    def _ws_evaluate(
        ...
        connect_packet = {
            ...
            "param": {
                "app": {
                    ...
                    "userId": user_id,
                    "sig": self._packet_sig(ts_connect),
                },
                "sdk": sdk,
            },
        }
        ...
        try:
            ...
            # 3. binary audio frames。未压缩音频必须按接近实时的速度灌入，
            # 不能用 4KB/20ms（约 6.4 倍速），否则引擎会提前关闭连接。
            stage = "发送音频"
            chunk_bytes = (
                PCM_FRAME_BYTES
                if audio_type in {"pcm", "wav"}
                else COMPRESSED_AUDIO_CHUNK_BYTES
            )
            ...
            raise SsecpError(
                f"SSECP 服务端在“{stage}”阶段关闭了连接，请检查 App 权限、题型和音频格式"
            ) from e
        finally:
            try:
                ws.close()
