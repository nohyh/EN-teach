...
def _speech_call(operation):
    try:
        return operation()
    except MissingSpeechConfigError as e:
        raise HTTPException(status_code=503, detail=str(e)) from e
    except (SpeechError, ValueError) as e:
        raise HTTPException(status_code=502, detail=f"发音服务暂时不可用: {e}") from e
...
def synth_for_sentence(
    ...
    audio = _speech_call(lambda: get_speech_service().synthesize(sentence.text, fmt=fmt))
    return Response(
        content=audio,
...
def synth_multiple_sentences(
    ...
    if pause_ms > 0 and fmt != "mp3":
        audio = _speech_call(lambda: svc.synth_multi(texts, pause_ms=pause_ms, fmt=fmt))
    else:
        chunks = _speech_call(lambda: [svc.synthesize(t, fmt=fmt) for t in texts])
        audio = b"".join(chunks)
    return Response(
        content=audio,
        media_type={"mp3": "audio/mpeg", "wav": "audio/wav", "pcm": "audio/L16"}[fmt],
